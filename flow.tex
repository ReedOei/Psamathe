...
\begin{tabular}{l r l l}
    ...
    $\Stmt$ & \bnfdef & $F$ \bnfalt $E$ \bnfalt $\revert(E)$ \bnfalt $\tryS~S~\catchS(x : \tau)~S$ \bnfalt $\ifS~x~\thenS~S~\elseS~S$ \\
            & \bnfalt & $\var~x:\tau$ \bnfalt $\var~x:\tau := E$ \bnfalt $S;S$ \bnfalt \pack \bnfalt $\unpack(x)$ & (statements) \\
...
\begin{mathpar}
    ...
    }{ \Gamma \flowproves (\var~x : \emptyq~T)~\wf \flowprovesout \Gamma, x : \emptyq~T }
    ...
    \inferrule*[right=Wf-Pack]{
        % \forall x \in \dom(\Gamma). \fields(C)(x) \leq \Gamma(x)
        \fields(C) \leq \Gamma
    }{ \Gamma, \this : C \flowproves \pack~\wf \flowprovesout \Gamma \setminus \dom(\fields(C)) }
\end{mathpar}
...
\begin{mathpar}
    ...
    \inferrule*[right=Wf-Tx]{
        \Gamma = \this : C, \fields(C), \overline{x : \tau}
        \and
        \Gamma \flowproves S~\wf \flowprovesout \Gamma
    }{ \proves_C (\transaction~m(\overline{x : \tau})~\returns~\sigma~\doC~S)~\wf }
\end{mathpar}
