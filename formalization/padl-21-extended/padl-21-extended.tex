...
\reed{NOTE: I considered going with the below; I decided against it (for now) because some rules (e.g., \textsc{Filter}) require ``passing'' the mode around
\framebox{$\Gamma \flowproves \flowfromto{f} \Loc : \tau \flowprovesout \Delta $}}
...
for all $1 \leq i \leq n$, $\Gamma_i \flowproves_M f_i ; \Loc_i : \tau_i \flowprovesout \Delta_i$ so that $\Gamma = \Gamma_1$, and $\Delta = \Delta_n$.
...
We use $\updateElem$ again, because it captures the difference \reed{I think...} between selecting the whole record and selecting parts of the fields of the record.
\begin{mathpar}
...
We must ensure that the predicate, $p$, really is a predicate: it accepts values of type $\elemtype(T)$ and returns $\exactlyone~\boolt$.
...
We arbitrarily choose $\emptyq$ as the type quantity for $\consume$.
\begin{mathpar}
...
This judgement states that in the environment $\Gamma$, the statement $S$ is well-formed and it transforms $\Gamma$ into $\Delta$.
...
\begin{mathpar}
    \inferrule*[right=Ok-Transformer]{
        \overline{x : \tau}, y : \tau_y, z : \emptyq~T_z \flowproves \overline{\Stmt~\ok} \flowprovesout \Delta, \overline{x : \tau}, z : \mathcal{Q}_z~T_z
        \\
        \forall v : \sigma \in \Delta. \lnot(\sigma~\asset)
    }{ (\transformer f(\overline{x : \tau}, y : \tau_y) \to \mathcal{Q}_z~T_z~\{ \overline{\Stmt} \}) ~\ok }
\end{mathpar}
