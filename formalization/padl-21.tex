...
An extended version~\cite{psamatheExtended} of this work explains the language in more detail, and shows more examples, such as a voting contract, highlighting the advantages we describe below.
...
type User is unique { name : string, password : string }
var users : set User <-- new User("foo", "bar")
var admin : User <-- users[one such that isAdmin(_)]
TODO: would like to showcase unique somehow
\end{lstlisting}
