...
\reed{NOTE: I considered going with the following (or something like it): $\Gamma \flowproves \flowfromto{f} \Loc : \tau \flowprovesout \Delta $, $\Gamma \flowproves \flowfrom{f} \Loc : \tau \flowprovesout \Delta $, and $\Gamma \flowproves \flowto{f} \Loc : \tau \flowprovesout \Delta $.
I decided against it (for now) because some rules (e.g., \textsc{Filter}) require ``passing'' the mode around.}
