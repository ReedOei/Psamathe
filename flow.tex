...
\begin{tabular}{l r l l}
    ...
    $\tau$ & \bnfdef & \boolt \bnfalt $\natt$ \bnfalt $\mathcal{Q}~\tau$ \bnfalt $\mathcal{Q}~\tau \times \mathcal{Q}~\tau$ \bnfalt $\map~\mathcal{Q}~\tau \Rightarrow \mathcal{Q}~\tau$ \bnfalt $\curlys{\overline{x : \mathcal{Q}~\tau}}$ \bnfalt $t$ & (types) \\
...
The syntax $f : \tau \flowsto \sigma$ means $f$ accepts $\tau$ and provides $\sigma$; that is, you can flow $\tau$ into it, and when you flow out of it, you get $\sigma$.
...
\begin{mathpar}
    ...
    \inferrule*[right=Lookup-NoCosume]{
    ...
    \inferrule*[right=Flow-Transformer]{
        \Gamma, x : \tau \flowproves e : \sigma \flowproveout \Gamma
    }{ \Gamma \flowproves (\lambda x : \tau. e) : \tau \flowsto \sigma }

    \inferrule*[right=Flow]{
        \Gamma \proves S : \mathcal{Q}~\tau
        \and
        \Gamma \proves f~\selects~\demote(\tau)
        \and
        \Gamma \proves D : \tau \flowsto \sigma
    }{ \Gamma \proves S \sends{f} D : \tau \flowsto \sigma }

    \inferrule*[right=Flow-Experimental]{
        \Gamma \proves S : \mathcal{Q}~\tau
        \and
        \Gamma \proves f ~ \selects~\tau
        \and
        \Gamma \flowproves D : \tau \flowsto \sigma \flowproveout \Delta
    }{ \Gamma \flowproves S \sends{f} D : \tau \flowsto \sigma \flowproveout \Delta }

    \inferrule*[right=Storage-Lookup]{
        \Gamma \proves \mathcal{S} : \one~\map~\tau~\Rightarrow~\sigma
        \and
        \Gamma \proves E : \demote(\tau)
    }{ \Gamma \proves \mathcal{S}[E] : \sigma }

%     \inferrule*[right=Nonfungible-Lookup]{
%         \tau ~ \nonfungible
%     }{ \Gamma, x : \mathcal{Q}~\tau \flowproves x : \mathcal{Q}~\tau \flowproveout \Gamma }

%     \inferrule*[right=Lookup]{
%         \lnot (\tau ~ \nonfungible)
%     }{ \Gamma, x : \mathcal{Q}~\tau \flowproves x : \mathcal{Q}~\tau \flowproveout \Gamma, x : \mathcal{Q}~\tau }
\end{mathpar}

\reed{I think the only kind of expression that will actually behave linearly is the expressions in the transformers.
And also the external calls, when we give a resource to them, which I suppose is possible?
Also depends what it's flowing to---if you flow from an option to a set, it's fine if it doesn't contain anything.
Everything else works ``normally''?
We could also guard on things like $A \to B; A \to B$ where $A : \one~\tau$, because the second one will always fail.
...
Probably would be nice to check for, we need to worry about this for all $\mathcal{Q}$ except for $\setof$; but also only for nonfungible types.
