...
\begin{align*}
    ...
    a,x,y,z &\in \textsc{Identifiers} & \alpha, \beta &\in \textsc{TypeVars} \\
\end{align*}
\begin{tabular}{l r l l}
    ...
    $T$ & \bnfdef & $\boolt$ \bnfalt $\natt$ \bnfalt $\alpha$ \bnfalt $\type~t~\is~\overline{M}~T$ \bnfalt $\listq~\tau$ \bnfalt $\{ \overline{x : \tau} \}$ & (base types) \\
    ...
    $\Decl$ & \bnfdef & $\transformer~f[\overline{\alpha}](\overline{x : \tau}) \to x : \tau ~\{ \overline{\Stmt} \}$ & (transformers) \\
