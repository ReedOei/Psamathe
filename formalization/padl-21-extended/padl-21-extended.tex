...
We use $f$ to refer to a function on types ($\tau \to \tau$), and we write $\id{\tau}$ for the identity function.
...
\begin{mathpar}
    ...
    \inferrule*[right=List]{
        \Gamma \flowproves_S \overline{L : \tau ; f} \flowprovesout \Delta
    }{ \Gamma \flowproves_S [ \tau ; \overline{L} ] : \#(|\overline{L}|)~\tableT(\cdot)~\tau ; f \flowprovesout \Delta }

    \inferrule*[right=Filter]{
        \Gamma \flowproves_M \Loc : \mathcal{R}~T ; (\tau \mapsto \tau \ominus \mathcal{Q}) \flowprovesout \Delta
        \\
    ...
    }{ \Gamma \flowproves_M \Loc[\mathcal{Q}~\tsuchthat~f(\overline{\LocK})] : \mathcal{Q}~T ; f \flowprovesout \Delta }
