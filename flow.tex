...
\begin{mathpar}
    \inferrule*[right=Empty-Val]{
        \emptyq \sqsubseteq \mathcal{Q}
    }{ \Gamma \flowproves \emptyq : \mathcal{Q}~\mathcal{C}~\tau \flowprovesout \Gamma }
    ...
    }{ \Gamma \proves (f~\varin~x) : \boolt \flowprovesout \Gamma }
    ...
    \inferrule*[right=Let]{
        \Gamma \flowproves E_1 : \tau \flowprovesout \Delta
        \and
        \Delta, x : \tau \flowproves E_2 : \sigma \flowprovesout \Xi
        \and
        x \not\in \dom(\Xi)
    }{ \Gamma \flowproves (\letvar~x : \tau := E_1~\varin~E_2) : \sigma \flowprovesout \Xi }
...
\begin{mathpar}
    ...
    \inferrule*[right=Store-New]{
        ...
        \and
        \Gamma \flowproves x : \curlys{\overline{\this.f : \tau}} \flowprovesout \Gamma
    }{ \Gamma \proves \newc~C(x) :: \voidt \flowsto_{\exactlyone} ~ C }

    \inferrule*[right=Store-New-Ty]{
        (\type~t~\is~\overline{M}~T) \in \decls(C)
        \and
        \Gamma \flowproves x : \demote(T) \flowprovesout \Gamma
    }{ \Gamma, \this : C \proves \newc~t(x) :: \voidt \flowsto_{\exactlyone} ~ t }
\end{mathpar}
...
\begin{mathpar}
    ...
    }{ \Gamma \flowproves (A \sends{s} B)~\wf \flowprovesout \update(\Delta, B, \Delta(B) \oplus \min(\mathcal{Q}, \mathcal{R})) }

    \inferrule*[right=Wf-Var-Def]{
        \Gamma \flowproves E : \tau \flowprovesout \Delta
        \and
        \Delta, x : \tau \flowproves S~\wf \flowprovesout \Xi, x : \sigma
        \and
        \lnot(\sigma~\asset)
    }{ \Gamma \flowproves (\var~x : \tau := E~\varin~S)~\wf \flowprovesout \Xi }
