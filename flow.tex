...
\begin{tabular}{l r l l}
    $\mathcal{Q}$ & \bnfdef & $\emptyq$ \bnfalt $\one$ \bnfalt $\optionq$ \bnfalt $\setq$ \bnfalt $\listq$ & (type quantities \reed{Not sure what to call these}) \\
...
\reed{How to handle transfer everything making it safe to ignore things?}
...
\begin{mathpar}
    ...
    \inferrule*[right=Storage]{
    }{ \Gamma, S : \mathcal{Q}~\tau \proves S : \tau \flowsto \tau }

    \inferrule*[right=Transformer]{
        \Gamma, x : \tau \flowproves e : \sigma \flowproveout \Gamma
    }{ \Gamma \proves (\lambda x : \tau. e) : \tau \flowsto \sigma }

    \inferrule*[right=Flow]{
        \Gamma \proves S : \tau \flowsto \sigma
        \and
        \Gamma \proves f~\selects_\mathfrak{q}~\demote(\sigma)
        \and
        \Gamma \proves D : \sigma \flowsto \pi
    }{ \Gamma \proves (S \sends{f} D) : \tau \flowsto \pi }
...
\framebox{$\Gamma \flowproves e : \tau \flowproveout \Delta$} \textbf{Linear Expression Typing}
\begin{mathpar}
    ...
    \inferrule*[right=Lin-View]{
        ...
        \and
        \lnot (\tau ~ \asset)
    }{ \Gamma \flowproves x : \tau \flowproveout \Gamma }

    \inferrule*[right=Lin-Weaken]{
        \Gamma \flowproves e : \sigma \flowproveout \Delta
        \and
        \lnot (\tau ~ \asset)
    }{ \Gamma, x : \tau \flowproves e : \sigma \flowproveout \Delta }
\end{mathpar}

\reed{IN interpreter, store flow activation for handlers}

\framebox{$\Gamma \flowproves S~\wf \flowproveout \Delta$} \textbf{Statement Well-formedness}
\begin{mathpar}
    \inferrule*[right=Wf-Flow]{
        \Gamma \proves F : \tau \flowsto \sigma
    }{ \Gamma \flowproves F~\wf \flowproveout F[\Gamma] }

    \inferrule*[right=Wf-Var-Def]{
    }{ \Gamma \flowproves \var~x : \tau \flowproveout \Gamma, x : \emptyq~\tau }

    \inferrule*[right=Wf-Seq]{
        \Gamma \flowproves S_1~\wf \flowproveout \Delta
        \and
        \Delta \flowproves S_2~\wf \flowproveout \Xi
    }{ \Gamma \flowproves S_1~;~S_2 \flowproveout \Xi }
\end{mathpar}
...
Let $F$ be a flow, $x$ an identifier and $\mathcal{Q}~\tau$ a type in canonical form (i.e., if $\tau = \mathcal{R}~\sigma$ then $\mathcal{R}' = \one$).
\begin{definition}
...
\begin{definition}
    \emph{$F$ fills $x : \mathcal{Q}~\tau$} (\emph{$F~\fillsarr~x:\mathcal{Q}~\tau$}) when $F = S \sends{f} x$ and $\Gamma \proves f~\selects_\mathfrak{q}~\tau$ where $! \leq \mathfrak{q}$.
\end{definition}

\framebox{$F[\Gamma] = \Delta$} \textbf{Storage Tracking}
For a flow $F$ and an environment $\Gamma$, $F[\Gamma]$ is the new state of the environment after the flow $F$ is performed.
Specifically, it upates which storages are guaranteed to be empty and which are guaranteed to be nonempty.
\begin{align*}
    F[\Gamma] &= \setbuild{x : \tau \in \Gamma}{\lnot (F~\empties~x)} \\
                          &\cup \setbuild{x : \emptyq~\tau}{x : \tau \in \Gamma, F~\consumesarr~x:\tau} \\
                          &\cup \setbuild{x : \fullq~\tau}{x : \tau \in \Gamma, F~\fillsarr~x:\tau}
\end{align*}
% If $\tau = \mathcal{Q}~\sigma$ for some $\sigma$ and $\mathcal{Q}$ such that $\mathcal{Q} \neq \one$, then $\emptyq~\tau = \tau$.
% \reed{Actually, we only end up using $\emptyq$ on things that are just $\one~\tau$ anyway, so this doesn't really matter.}

\reed{handle $\consume~\this$}

\reed{Probably should only be possible to transfer resources via. ``external'' calls to calls to the same contract.}
\reed{We could also guard on things like $A \to B; A \to B$ where $A : \one~\tau$, because the second one will always fail.
Not sure if we care to put this in the type system, or just have a linting check.
It's safe either way because of dynamic checks.
Probably would be nice to check for, we need to worry about this for all $\mathcal{Q}$ except for $\setq$; but also only for nonfungible types.
Fungible types can be flowed from infinitely many times, the later flows just don't do anything.
That seems like a better candidate for a lint check.}
...
\reed{NOTE: Transformers \textbf{must} be consumed, because they might be holding resources if partially applied.
