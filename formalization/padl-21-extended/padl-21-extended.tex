...
We may type a $\natt$ with any quantity that is compatible with it's actual quantity; for example, if $n = 1$, we can use $\exactlyone$, $\nonempty$, or $\any$, but if $n = 0$, we cannot use $\nonempty$.
\begin{mathpar}
    \inferrule*[right=Nat]{
        \#(n) \sqsubseteq \mathcal{Q}
    }{ \Gamma \flowproves_S n : \mathcal{Q}~\natt; f \flowprovesout \Gamma }
...
Variable definitions must be destinations, as newly defined variables are always empty, so there is no reason to use them as sources. \reed{though it's not strictly \textbf{wrong}}
\begin{mathpar}
...
\reed{TODO: Need to ensure that $f$ works on list elements, not the whole list (otherwise we need to ``translate'' it)}
\begin{mathpar}
    \inferrule*[right=List]{
        \Gamma \flowproves_S \overline{\Loc : \tau ; f} \flowprovesout \Delta
        \\
        \#(|\overline{\Loc}|) \sqsubseteq \mathcal{Q}
    }{ \Gamma \flowproves_S [ \tau ; \overline{\Loc} ] : \mathcal{Q}~\tableT(\cdot)~\tau ; f \flowprovesout \Delta }
\end{mathpar}
...
\begin{mathpar}
    \inferrule*[right=Filter]{
        \transformer f(\overline{x : \tau}, y : \elemtype(T)) \to \one~\boolt~\{ \overline{\Stmt} \}
        \\
        \Gamma \flowproves_M \Loc : \mathcal{R}~T ; (\tau \mapsto \max(f(\tau), \tau \ominus \mathcal{Q})) \flowprovesout \Delta
        \\
        \mathcal{R} \geq \mathcal{Q}
        \\
        \Delta \flowproves \overline{\LocK : \tau} ; (\mathcal{Q}'~T' \mapsto \emptyq~T') \flowprovesout \Delta
    }{ \Gamma \flowproves_M \Loc[\mathcal{Q}~\tsuchthat~p(\overline{\LocK})] : \mathcal{Q}~T ; f \flowprovesout \Delta }
\end{mathpar}
\reed{TODO: It might actually be good to let function types say which arguments can be changed; if we do this, then we can have a less restrictive check on the types of arguments in \textsc{Filter}}
...
\begin{mathpar}
    ...
    }{ \angles{ \Sigma, b } \to \angles{ \Sigma[\rho \mapsto \rho[\ell \mapsto (\boolt, b)]], (\ell, \ell) } }
