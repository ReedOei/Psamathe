...
\begin{tabular}{l r l l}
    ...
    $\mathcal{Q}$ & \bnfdef & $\mathfrak{q}$ \bnfalt $\bm{0}$ \bnfalt $?$ \bnfalt $\infty$ & (\typeQuantities \reed{Not sure what to call these}) \\
    $T$ & \bnfdef & $\voidt$ \bnfalt \boolt \bnfalt $\natt$ \bnfalt $\optionq~\tau$ \bnfalt $\listq~\tau$ \bnfalt $\setq~\tau$ \bnfalt $\tau \times \tau$ \bnfalt $T \transforms T$ \bnfalt $\curlys{\overline{x : \tau}}$ \bnfalt $t$ & (base types) \\
    $\tau$ & \bnfdef & $\mathcal{Q}~T$ & (types) \\
    $\mathcal{V}$ & \bnfdef & $n$ \bnfalt \true \bnfalt \false \bnfalt $\lambda x : \tau. E$ & (values) \\
...
In the surface language, ``container types'' like $\setq$ and $\listq$ are by default $*$, but types like $\natt$ or $\natt \times \natt$ are $!$.
...
\reed{We could also get rid of ``if'' and instead do something like $*~x:\tau~\suchthat~\ifS~b~\thenS~x = y~\elseS~false$}
...
\begin{definition}
    Define $\textbf{Quant} = \curlys{\bm{0}, ?, !, *, +, \infty}$, and call any $\mathcal{Q} \in \textbf{Quant}$ a \emph{type quantity}.
    Define $\bm{0} \leq * \leq \,\, ? \leq \,\, ! \leq + \leq \infty$, based on the amount of resources that every selector quantifier is \textbf{guaranteed} to select.
    The elements are called \emph{none}, \emph{any}, \emph{at most one}, \emph{exactly one}, \emph{at least one}, and \emph{everything}, respectively.
...
\begin{mathpar}
    ...
    }{ \Gamma \proves (\lambda x : \tau. e) : \emptyq~(\tau \transforms \sigma) }
    ...
    \inferrule*[right=Call-View]{
        ...
        \and
        \view~m(\overline{a : \tau})~\returns~\sigma := E
        \and
        \Gamma \proves \overline{y : \tau}
    }{ \Gamma, x : C \proves x.m(\overline{y}) : \sigma }
    ...
    \inferrule*[right=If-Expr]{
        ...
        \and
        \Gamma \proves z : \tau
    }{ \Gamma \proves (\ifS~x~\thenS~y~\elseS~z) : \tau }
\end{mathpar}
...
% \begin{definition}
%     A base type $T$ is a \emph{container type} if it is of the form $\setq~T'$, $\listq~T'$, or $T_1 \transforms T_2$.
% \end{definition}

\begin{definition}
    For a type $\mathcal{Q}~T$, define $|\mathcal{Q}~T| = \mathcal{Q}$.
    % \begin{align*}
    %     |\emptyq~T| &= \bm{0} \\
    %     |\nonempty~T| &=
    %     \begin{cases}
    %         + & \tif T \text{is a container type} \\
    %         ! & \owise
    %     \end{cases} \\
    %     |\unknown~T| &=
    %     \begin{cases}
...
\framebox{$\Gamma \proves \mathcal{S} :: T_1 \flowsto_{\mathcal{Q}} T_2$} \textbf{Storage Typing}
The syntax $\tau \flowsto_{\mathfrak{q}} \sigma$ means that the storage accepts $\tau$ and provides $\sigma$; that is, you can flow $\tau$ into it, and when you flow out of it, you get $\sigma$; moreover, you get at most $\mathfrak{q}$ of them.
\begin{mathpar}
    \inferrule*[right=Store-One]{
    }{ \Gamma, S : \mathcal{Q}~T \proves S :: T \flowsto_{\mathcal{Q}} T }

    \inferrule*[right=Store-Col]{
        g \in \curlys{\optionq, \setq, \listq}
    }{ \Gamma, S : \mathcal{Q}~g~T \proves S :: T \flowsto_{\mathcal{Q}} T }

    \inferrule*[right=Store-Transformer]{
    }{ \Gamma, x : \mathcal{Q}~(T_1 \transforms T_2) \proves x :: T_1 \flowsto_{\mathcal{Q}} T_2 }

    \inferrule*[right=Store-Consume]{
        T~\consumable
    }{ \Gamma \proves \consume :: T \flowsto_{\bm{0}} \voidt }
...
\framebox{$\Gamma \proves f ~\selects_\mathcal{Q}~ T$} \textbf{Selectors}
\begin{mathpar}
    \inferrule*[right=Select-Expr]{
        \Gamma \proves E : \mathcal{Q}~T
    }{ \Gamma \proves E~\selects_{\mathcal{Q}}~T }

    \inferrule*[right=Select-Everything]{
    }{ \Gamma \proves \everything~\selects_\infty~T }
...
\reed{Should we combine linear and regular expression typing, and then for regular expression stuff just don't let the output type env. change?}

\begin{definition}
    Let $\mathcal{Q}$ and $\mathcal{R}$ be selector quantifiers.
    Define the commutative operator \emph{combine}, written $\oplus$, as the unique function $\textbf{Quant}^2 \to \textbf{Quant}$ satisfying
    \begin{align*}
        \infty \oplus \mathcal{R} &= \infty \\
        \bm{0} \oplus \mathcal{R} &= \mathcal{R} \\
        + \oplus \mathcal{R} &= + \\
        ! \oplus \mathcal{R} &= + \qquad \tif \mathcal{R} \neq \bm{0} \\
        ? \oplus ? &= * \\
        * \oplus * &= *
    \end{align*}

    Similarly, define the operator \emph{split}, written $\ominus$, as the unique function $\textbf{Quant}^2 \partialfunc \textbf{Quant}$ such that
    \begin{align*}
        \mathcal{Q} \ominus \bm{0} & = \mathcal{Q} \\
        \bm{0} \ominus \mathcal{R} & = \bm{0} \\
        ! \ominus \mathcal{R} &=
        \begin{cases}
            \bm{0} & \tif \mathcal{R} \geq ! \\
            ? & \owise
        \end{cases} \\
        + \ominus \mathcal{R} &= * \qquad \tif \mathcal{R} \neq \bm{0} \\
        \mathcal{Q} \ominus * &= * \\
        \mathcal{Q} \ominus \infty & = \bm{0} \\
...
\begin{definition}
    ...
    For types,
    \[
        \mathcal{Q}~T_\mathcal{Q} \leq \mathcal{R}~T_\mathcal{R} \iff T_\mathcal{Q} = T_\mathcal{R} \tand \mathcal{Q} \leq \mathcal{R}
    \]
...
\begin{mathpar}
    \inferrule*[right=Wf-Flow]{
        \Gamma \flowproves S :: T_1 \flowsto_\mathcal{Q} T_2
        \and
        \Gamma \flowproves f~\selects_\mathcal{R}~\demote(T_2)
        \\
        \Delta = \update(\Gamma, S, \Gamma(S) \ominus \mathcal{R})
        \and
        \Delta \proves D :: T_2 \flowsto_\mathcal{S} T_3
