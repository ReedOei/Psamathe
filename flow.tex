...
\begin{tabular}{l r l l}
    ...
    $E$ & \bnfdef & $\mathcal{V}$ \bnfalt $x$ \bnfalt \letvar $x : \tau := e$ \varin e \bnfalt $\mathcal{S}$ \\
    $f$ & \bnfdef & $E$ \bnfalt $x \%$ \bnfalt $\everything$ \bnfalt $\min(f, f)$ \bnfalt $\max(f, f)$ \bnfalt $\mathfrak{q}~x~\suchthat~E$ & (selector) \\
    $F$ & \bnfdef & $\mathcal{S} \sends{f} \mathcal{D}$ & (flows)
...
\framebox{$\combine(\mathfrak{q}, \tau)$} \textbf{Typing}
\begin{align*}
