...
\begin{tabular}{l r l l}
    ...
    $\mathcal{S}$ & \bnfdef & $x$ \bnfalt $\mathcal{S}[E]$ \bnfalt $\mathcal{S}.x$ & (storage identifiers) \\
...
\framebox{$\combine(\mathfrak{q}, \tau)$} \textbf{Storage modification}
These auxiliary functions are used to update the statically known information about a storage after flowing in or out of it, see the flow typing rules.
...
\begin{mathpar}
    ...
    \inferrule*[right=Flow]{
        ...
        \\
        \Gamma[S \leftarrow \select(f, \mathfrak{q}, \Gamma(S))] \flowproves D :: \sigma \flowsto \pi \flowprovesout \Delta
    }{ \Gamma \flowproves (S \sends{f} D) :: \tau \flowsto \pi \flowprovesout \Delta[D \leftarrow \combine(\mathfrak{q}, \Delta(D))] }
\end{mathpar}

Note that $\Gamma[S \leftarrow \tau]$ is the environment $\Gamma$ after updating the type of $S$ to be $\tau$.
