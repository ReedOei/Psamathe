...
\begin{tabular}{l r l l}
    ...
    $\mathcal{Q}$ & \bnfdef & $\mathfrak{q}$ \bnfalt $\emptyq$ \bnfalt $?$ \bnfalt $\infty$ & (\typeQuantities \reed{Not sure what to call these}) \\
...
\begin{definition}
    Define $\textbf{Quant} = \curlys{\emptyq, ?, !, *, +, \infty}$, and call any $\mathcal{Q} \in \textbf{Quant}$ a \emph{type quantity}.
    Define $\emptyq \leq * \leq \,\, ? \leq \,\, ! \leq + \leq \infty$, based on the amount of resources that every selector quantifier is \textbf{guaranteed} to select.
...
\begin{mathpar}
    ...
    }{ \Gamma \flowproves (\lambda x : \tau. E) : \emptyq~(\tau \transforms \sigma) \flowprovesout \Gamma }
...
\begin{mathpar}
    ...
    \inferrule*[right=Store-Consume]{
        T~\consumable
    }{ \Gamma \proves \consume :: T \flowsto_{\emptyq} \voidt }
