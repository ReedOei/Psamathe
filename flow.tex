...
\begin{tabular}{l r l l}
    ...
    $T$ & \bnfdef & $\voidt$ \bnfalt \boolt \bnfalt $\natt$ \bnfalt $\optionq~\tau$ \bnfalt $\listq~\tau$ \bnfalt $\setq~\tau$ \bnfalt $\tau \times \tau$ \bnfalt $\tau \transforms \tau$ \bnfalt $\curlys{\overline{x : \tau}}$ \bnfalt $t$ & (base types) \\
    ...
    $S$ & \bnfdef & $F$ \bnfalt $\ifS~E~\thenS~S$ \bnfalt $S ; S$ \bnfalt $\var~x:\tau$ \bnfalt $\pack(C)$ & (statements) \\
...
In the surface language, ``container types'' like $\setq$ and $\listq$ are by default $\unknown$, but types like $\natt$ or $(\natt, \natt)$ are $\nonempty~\natt$.
\reed{Not really sure about this...}
...
\reed{We could also get rid of if and such and instead do stuff like $*~x:\tau~\suchthat~\ifS~b~\thenS~x = y~\elseS~false$}
...
\begin{definition}
    For a \typeQuantity $\mathcal{Q}$, define $|\mathcal{Q}|$ by
    \begin{align*}
        |\mathcal{Q}| =
        \begin{cases}
            + & \tif \mathcal{Q} = \nonempty \\
...
\begin{mathpar}
    ...
    \inferrule*[right=Forget]{
        ...
        \and
        \mathcal{Q} \leq \mathcal{R}
    }{ \Gamma \proves x : \mathcal{R}~\tau }

    \inferrule*[right=Call-View]{
        \text{\reed{Finish}}
    }{ \Gamma \proves m(x_1, \ldots, x_n) : \tau }

    \inferrule*[right=If-Expr]{
        \text{\reed{Finish}}
    }{ \Gamma \proves (\ifS~x~\thenS~y~\elseS~z) : T }
\end{mathpar}
...
\framebox{$\Gamma \proves f ~\selects_\mathfrak{q}~ \tau$} \textbf{Selectors}
\begin{mathpar}
