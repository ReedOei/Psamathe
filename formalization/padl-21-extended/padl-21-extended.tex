...
\reed{TODO: Write how this should be read (same for other judgements)}
...
We use $f$ to refer to a function on types ($\types \to \types$), and we write $\id{\types}$ for the identity function on types.
...
\begin{mathpar}
    ...
    }{ \Gamma \flowproves_S b : \exactlyone~\boolt; f \flowprovesout \Gamma }
\end{mathpar}
...
\begin{mathpar}
    \inferrule*[right=Subquant]{
        \Gamma \flowproves_M \Loc : \mathcal{R}~T ; f \flowprovesout \Delta
        \\
        \mathcal{R} \sqsubseteq \mathcal{Q}
    }{ \Gamma \flowproves_M \Loc : \mathcal{Q}~T ; f \flowprovesout \Delta }
\end{mathpar}
...
\begin{mathpar}
    \inferrule*[right=Var]{
        f = \id{\types} \tor \lnot(\tau~\immutable)
    }{ \Gamma, x : \tau \flowproves_M (x : \tau) ; f \flowprovesout \Gamma, x : f(\tau) }
\end{mathpar}
...
\begin{mathpar}
    ...
    }{ \Gamma \flowproves_S \copyC(\Loc) : \demoteT(\tau); f \flowprovesout \Gamma }
...
\begin{mathpar}
    \inferrule*[right=List]{
        \Gamma \flowproves_S \overline{\Loc : \tau} ; \updateElem(f) \flowprovesout \Delta
    }{ \Gamma \flowproves_S [ \tau ; \overline{\Loc} ] : \#(|\overline{\Loc}|)~\tableT(\cdot)~\tau ; f \flowprovesout \Delta }
\end{mathpar}

where
\reed{I think this is right, but it's a little messy.}
\[
    \updateElem(f) =
    \begin{cases}
        f & \tif f = \id{\types} \tor f = \lambda \mathcal{Q}~T. \emptyq~T \\
        \lambda \mathcal{Q}~T. (\mathcal{Q} \sqcup \emptyq)~T & \owise
    \end{cases}
\]
...
We use $\lambda \mathcal{Q}'~T'. \max(f(\mathcal{Q}'~T'), (\mathcal{Q}' \ominus \mathcal{Q})~T')$ to capture whether $f$ is $\id{\types}$ or performs some modification.
...
\begin{mathpar}
    \inferrule*[right=Filter]{
        ...
        \\
        \Gamma \flowproves_M \Loc : \mathcal{R}~T ; (\lambda \mathcal{Q}'~T'. \max(f(\mathcal{Q}'~T'), (\mathcal{Q}' \ominus \mathcal{Q})~T') \flowprovesout \Delta
        \\
        \mathcal{R} \geq \mathcal{Q}
        \\
        \Gamma \flowproves \overline{\LocK : \tau} ; \id{\types} \flowprovesout \Gamma
    }{ \Gamma \flowproves_M \Loc[\mathcal{Q}~\tsuchthat~p(\overline{\LocK})] : \mathcal{Q}~T ; f \flowprovesout \Delta }
\end{mathpar}

We may only use $\consume$ as a destination, and only if $T$ is a consumable type.
We arbitrarily choose $\emptyq$ as the type quantity for $\consume$. \reed{I don't think it matters at all which one is chosen}
\begin{mathpar}
    \inferrule*[right=Consume]{
        T~\consumable
    }{ \Gamma \flowproves_D \consume : \emptyq~T ; f \flowprovesout \Gamma }
\end{mathpar}
...
All the values selected by $\Loc$ are cleared out and added to $\LocK$, using the updaters.
\begin{mathpar}
    \inferrule*[right=Ok-Flow]{
        \Gamma \flowproves_S \Loc : \mathcal{Q}~T ; (\lambda \mathcal{Q}'~T'. \emptyq~T') \flowprovesout \Delta
        \\
        \Delta \flowproves_D \LocK : \mathcal{R}~T ; (\lambda \mathcal{Q}'~t'. (\mathcal{Q}' \oplus \mathcal{Q})~T') \flowprovesout \Xi
    }{ \Gamma \flowproves (\Loc \to \LocK)~\ok \flowprovesout \Xi }
\end{mathpar}

\framebox{$\flowproves f : (\overline{x : \tau}) ; y : \sigma \transforms z : \pi$} \textbf{Transformer Typing}
\reed{TODO: I think, here, we also need to ensure that not only do the arguments that don't change have the same type, they can never be \emph{moved} (i.e., appear ``bare'' on the LHS of a flow)}
...
\begin{mathpar}
    \inferrule*[right=Ok-Transformer]{
        \overline{x : \tau}, y : \mathcal{Q}_y~T_y, z : \emptyq~T_z \flowproves \overline{\Stmt~\ok} \flowprovesout \Delta, \overline{x : \tau}, z : \mathcal{Q}_z~T_z
        \\
        \forall v : \sigma \in \Delta. \lnot(\sigma~\asset)
    }{ (\transformer f(\overline{x : \tau}, y : \mathcal{Q}_y~T_y) \to \mathcal{Q}_z~T_z~\{ \overline{\Stmt} \}) ~\ok }
\end{mathpar}
...
\begin{mathpar}
    ...
    \inferrule*[right=Loc-Bool]{
        n \not\in \dom(\rho)
    }{ \angles{ \Sigma, b } \to \angles{ \Sigma[\rho \mapsto \rho[\ell \mapsto (\boolt, b)]], (n, n) } }
...
\begin{mathpar}
    \inferrule*[right=Flow]{
        ...
        \\
        \rho(m) + R \neq \error
    }{ \angles{ \Sigma, (n, \ell) \to (m, k) } \to \Sigma[\rho \mapsto \rho[n \mapsto \rho(n) - R, m \mapsto \rho(m) + R]] }
    ...
    \inferrule*[right=Flow-List]{
        \overline{\select(\rho, n, \ell) = R}
    }{ \angles{ \Sigma, [ \tau ; \overline{(n, \ell)} ] \to (m,k) } \to \Sigma[\rho \mapsto \rho[n \mapsto \rho(n) - R, m \mapsto \rho(m) + R]] }
\end{mathpar}

\reed{TODO: Need to finish \textsc{Flow-List} (needs to prperly select and remove and add)}

\framebox{$\Gamma \compat \Sigma$} \textbf{Environment compatibility}
\reed{This is only necessary for the proofs}
...
\begin{mathpar}
    \inferrule*[right=Compat]{
        \dom(\Gamma) = \dom(\mu) \\
        \forall x : \tau \in \Gamma. \exactType(\select(\rho, \mu(x))) \sqsubseteq_{\tau} \tau
    }{ \Gamma \compat \Sigma }
\end{mathpar}
...
\reed{TODO: For some reason the references stopped showing up...???}
