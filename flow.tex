...
\begin{definition}
    ...
    \[
        \mathcal{Q} \sqsubseteq \any; \qquad \exactlyone \sqsubseteq \nonempty; \qquad \every \sqsubseteq \,\, \exactlyone
    \]
...
\begin{mathpar}
    ...
    \inferrule*[right=Wf-Var-Def]{
        T \neq \tau \transforms \sigma
        \and
        \Gamma, x : \emptyq~T \flowprovesout S~\wf \flowprovesout \Delta, x : \pi
        \and
        \lnot(\sigma~\pi)
    }{ \Gamma \flowproves (\var~x : \emptyq~T~\varin~S)~\wf \flowprovesout \Delta }
...
\begin{mathpar}
    ...
    \inferrule*[right=Wf-Tx]{
        \this : C, \fields(C), \overline{x : \tau} \flowproves S~\wf \flowprovesout \Delta, \this : C
        \and
        \Delta, \this : C \proves y : \sigma \flowprovesout \Delta, \this : C
        \\
        \dom(\fields(C)) \cap \dom(\Delta) = \emptyset
        \and
        \forall x : \tau \in \Delta. \lnot(\tau~\asset)
    }{ \proves_C (\transaction~m(\overline{x : \tau})~\returns~y : \sigma~\doC~S)~\wf }
\end{mathpar}
