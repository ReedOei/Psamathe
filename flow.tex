...
\begin{tabular}{l r l l}
    ...
    $E$ & \bnfdef & $\mathcal{V}$ \bnfalt $\mathcal{L}$ \bnfalt $x.m(\overline{x})$ \bnfalt $\letvar~x:\tau := E~\varin~E$ \bnfalt $f \varin~x$ & (expressions) \\
    ...
    $F$ & \bnfdef & $\ell: \mathcal{N} \sends{s} \mathcal{N}$ & (flows) \\
    $\Stmt$ & \bnfdef & $x.m(\overline{x})$ \bnfalt $\revert(E)$ \bnfalt $F$ \bnfalt $\tryS~S~\catchS(x : \tau)~S$ \bnfalt $\ifS~E~\thenS~S$ \bnfalt $\var~x:\tau$ \bnfalt $\var~x:\tau := E$ \bnfalt $S;S$ & (statements) \\
    % $A$ & \bnfdef & $\error(E)$ \bnfalt $\call~x.m(\overline{x})~\asserting~E$ \bnfalt $\emit~E(\overline{x})$ & (actions) \\
...
\reed{Contract types should be consumable assets by default (consuming a contract is a self-destruct?), transformers are always assets, because they could be partially applied and holding assets.}
...
\begin{mathpar}
    ...
    }{ \Gamma \flowproves (\ell : S \sends{s} D)~\wf \flowprovesout \update(\Delta, D, (\mathcal{Q} \wedge \mathcal{R}) \oplus \Delta(D)) }
