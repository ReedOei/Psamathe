...
\begin{mathpar}
    \inferrule*[right=Copy]{
        \Gamma \proves_S \Loc : \tau ; \id{\tau} \flowprovesout \Gamma
    }{ \Gamma \flowproves_S \copyC(\Loc) : \demoteT(\tau); f \flowprovesout \Gamma }

    % \inferrule*[right=Demote]{
    %     \Gamma \proves_S \Loc : \tau ; \id{\tau} \flowprovesout \Gamma
    % }{ \Gamma \flowproves_S \demote(\Loc) : \demoteT(\tau); \id{\tau} \flowprovesout \Gamma }
\end{mathpar}
...
\begin{mathpar}
    \inferrule*[right=Filter]{
        \flowproves p : (\overline{x : \tau}); y : \elemtype(T) \transforms \one~\boolt
        \\
        \Gamma \flowproves_M \Loc : \mathcal{R}~T ; (\tau \mapsto \max(f(\tau), \tau \ominus \mathcal{Q})) \flowprovesout \Delta
        \\
        \mathcal{R} \geq \mathcal{Q}
        \\
        \Delta \flowproves \overline{\LocK : \tau} ; \id{\tau} \flowprovesout \Delta
...
\begin{mathpar}
    \inferrule*[right=Transformer]{
        \transformer f(\overline{x : \tau}, y : \mathcal{Q}_y~T_y) \to \mathcal{Q}_z~T_z~\{ \overline{\Stmt} \}
        \\
    ...
    }{ \flowproves f : (\overline{x : \tau}) ; y : \mathcal{Q}_y~T_y \transforms z : \mathcal{Q}_z~T_z }
\end{mathpar}
...
\begin{mathpar}
    ...
    %     x : \tau, y : \emptyq~T \flowproves \overline{\Stmt~\ok} \flowprovesout \Delta, y : \mathcal{Q}~T
    %     \\
    %     \forall z : \sigma \in \Delta. \lnot(\sigma~\asset)
    % }{ \proves (\transformer f(\overline{x : \tau}) \to y : \mathcal{Q}~T ~\{ \overline{\Stmt} \})~\ok }
\end{mathpar}
