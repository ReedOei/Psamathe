...
\begin{mathpar}
    ...
    \inferrule*[right=Field]{
        ...
        \lnot(\tau~\immutable)
        \\
        \fields(\tau) = \overline{y : \sigma}
        \\
        f : \mathcal{R}~T \in \overline{y : \sigma}
    }{ \Gamma \flowproves x.f : \mathcal{R}~T \flowprovesout \update(\Gamma, x.f, \empty~T) }
    ...
    }{ \Gamma \flowproves \{ \overline{x : \tau \mapsto y} \} : \,\,\exactlyone~\{ \overline{x : \tau} \} \flowprovesout \Delta }
...
\begin{mathpar}
    ...
    \inferrule*[right=Ok-Flow-Every]{
        ...
        \\
        \Delta[\mathcal{S} \leftarrow \Delta(\mathcal{S}) \ominus \mathcal{Q}] \flowproves \mathcal{D} : \mathcal{R}~T \flowprovesout \Xi
    }{ \Gamma \flowproves (\mathcal{S} \to \mathcal{D})~\ok \flowprovesout \Xi[\mathcal{D} \leftarrow \Xi(\mathcal{D}) \oplus \mathcal{Q}] }

    \inferrule*[right=Ok-Flow-Filter]{
        \Gamma \flowproves \mathcal{S} : \mathcal{Q}~T \flowprovesout \Delta
        \\
        \transformer~f(\overline{x : \sigma}, y : \demote(\elemtype(T))) \to z : \,\,!~\boolt ~\doC~ \overline{\Stmt}
        \\
        \forall i. \demote(\Gamma(a_i)) = \sigma_i
        \\
        \Delta[\mathcal{S} \leftarrow \Delta(\mathcal{S}) \ominus \mathcal{Q}] \flowproves \mathcal{D} : \mathcal{S}~T \flowprovesout \Xi
    }{ \Gamma \flowproves (\mathcal{S} \sends{\mathcal{R}~\suchthat~f(\overline{a})} \mathcal{D})~\ok \flowprovesout \Xi[\mathcal{D} \leftarrow \Xi(\mathcal{D}) \oplus \min(\mathcal{Q}, \mathcal{R})] }

    \inferrule*[right=Ok-Flow-Transformer]{
        \Gamma \flowproves \mathcal{S} : \mathcal{Q}~T_1 \flowprovesout \Delta
        \\
        \transformer~f(\overline{x : \sigma}, y : \demote(\elemtype(T_1))) \to z : \mathcal{R}~T_2~\doC~ \overline{\Stmt}
        \\
        \forall i. \demote(\Gamma(x_i)) = \sigma_i
        \\
        \Delta[\mathcal{S} \leftarrow \Delta(\mathcal{S}) \ominus \mathcal{Q}] \flowproves \mathcal{D} : \mathcal{S}~T_2 \flowprovesout \Xi
    }{ \Gamma \flowproves (\mathcal{S} \to f(\overline{x}) \to \mathcal{D})~\ok \flowprovesout \Xi[\mathcal{D} \leftarrow \Xi(\mathcal{D}) \oplus \mathcal{Q}] }
...
\begin{tabular}{l r l l}
    ...
    $\Stmt$ & \bnfdef & $\ldots$ \bnfalt $\putS(\mathcal{V}, \mathcal{S})$ \bnfalt $\putS(\mathcal{V}, \mathcal{D})$ \bnfalt $\revert$ \bnfalt $\tryS(\Sigma, \overline{S}, \overline{S})$ & \\
\end{tabular}

\begin{definition}
    An environment $\Sigma$ is a tuple $(\mathcal{T}, \mu, \rho)$ where $\mathcal{T} : \textsc{TypeNames} \partialfunc \mathcal{V}$ is the \emph{source environment}, $\mu : \textsc{IdentifierNames} \partialfunc \mathbb{N}$ is the \emph{variable lookup environment}, and $\rho : \mathbb{N} \partialfunc \mathcal{V}$ is the \emph{storage environment}.
\end{definition}
...
\begin{mathpar}
    ...
    \inferrule*[right=Put-Var]{
        \rho(\mu(A)) = \mathcal{W}
    }{ \angles{ \Sigma, \putS(\mathcal{V}, A) } \evaluates \Sigma[\rho \leftarrow \rho[\mu(A) \leftarrow \mathcal{W}\mathcal{V}]] }

    \inferrule*[right=Put-VarDef]{
        \ell \not\in \dom(\rho)
    }{ \angles{ \Sigma, \putS(\mathcal{V}, \var~A : T) } \evaluates \Sigma[\mu \leftarrow \mu[A \leftarrow \ell], \rho \leftarrow \rho[\ell \leftarrow \mathcal{V}]] }

    \inferrule*[right=Flow-Every]{
        \resolve(\Sigma, \mathcal{S}) = (\Sigma', \ell)
    }{ \angles{\Sigma, \mathcal{S} \to \mathcal{D}} \evaluates \angles{\Sigma'[\rho \leftarrow \rho'[\ell \leftarrow []]], \putS(\rho'(\ell), \mathcal{D}) } }
    ...
    \inferrule*[right=Flow-Filter]{
        ...
        \\
        \compatQuan(|\mathcal{U}|, \mathcal{Q})
    }{ \Sigma, (\mathcal{S} \sends{\mathcal{Q}~\suchthat~f(\overline{x})} \mathcal{D}) \to \angles{ \Sigma'[\rho' \leftarrow \rho'[\ell \leftarrow \rho'(\ell) \setminus \mathcal{U}]], \putS(\mathcal{U}, \mathcal{D}) } }
    ...
    \inferrule*[right=Flow-Filter-Fail]{
        ...
        \mathcal{U} = [ v \in \mathcal{V} ~|~ \angles{\Sigma', f(\overline{x}, v)} \to^* \angles{\Sigma'', k} \tand \rho''(k) = \true ]
        \\
        \lnot \compatQuan(|\mathcal{U}|, \mathcal{Q})
    }{ \Sigma, (\mathcal{S} \sends{\mathcal{Q}~\suchthat~f(\overline{x})} \mathcal{D}) \to \angles{ \Sigma, \revert } }
    ...
    \inferrule*[right=Flow-Transformer]{
        ...
        \rho'(\ell) = v, \mathcal{V}
        \\
        \angles{ \Sigma'[\rho \leftarrow \rho'[\ell \leftarrow \mathcal{V}]], f(\overline{x}, v) } \to^* \angles{ \Sigma'', k }
    }{ \angles{\Sigma, \mathcal{S} \to f(\overline{x}) \to \mathcal{D}} \evaluates \angles{ \Sigma'', \putS(\rho(k), \mathcal{D}) ~ (\mathcal{S} \to f(\overline{x}) \to \mathcal{D}) } }
    ...
    \inferrule*[right=Call]{
        \ell \not\in \dom(\rho)
        \\
        \transformer~f(\overline{y : \tau}) \to z : \sigma~\doC~\overline{S}
    }{ \angles{ \Sigma, f(\overline{x}) } \evaluates \angles{ \Sigma'[\mu \leftarrow \mu[\overline{y \leftarrow \mu(x)}, z \leftarrow \ell], \rho \leftarrow \rho[\ell \leftarrow []]], \overline{S}~\ell } }
...
We use $\resolve(\Sigma, \mathcal{S})$ to get the location storing the values of $\mathcal{S}$, which returns a new environment because it may need to allocate new memory (e.g., in the case of $[x]$).
\reed{TODO: Finish the rest of the cases}
...
\begin{mathpar}
    ...
    \inferrule*[right=Resolve-Singleton]{
        \ell \not\in \dom(\rho)
    }{ \resolve(\Sigma, [ x ]) = (\Sigma[\rho \leftarrow \rho[\ell \leftarrow \rho(\mu(x)), \mu(x) \leftarrow []]], \ell) }
\end{mathpar}
...
\framebox{$\compatQuan(\mathbb{N} \cup \{ \infty \}, \mathcal{Q})$}
\begin{align*}
    \compatQuan(n, \mathcal{Q}) \iff & (\mathcal{Q} = \nonempty \tand n \geq 1) \tor \\
                                     & (\mathcal{Q} = \,\,\exactlyone \tand n = 1) \tor \\
                                     & (\mathcal{Q} = \emptyq \tand n = 0) \tor \\
                                     & (\mathcal{Q} \not\in \curlys{\nonempty, \exactlyone, \emptyq})
\end{align*}
